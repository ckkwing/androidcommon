	...
	public void process(FilterChain chain, Request request, Response response) {
		...
		try {
			...
			if (handler == null) {
				...
				String fileName = FileUtility.getFileName(request.getAbsoluteUrl());
				if (fileName.startsWith(".")) {
					switch (method) {
						...
						case LOCK:
						case UNLOCK:
							manager.getResponseHandler().respondForbidden(null, response, request);
							break;
						default:
					...
					//response.sethe("Connection", "keep-alive");
					...
					//response.setNonStandardHeader("Connection", "Close");
				}
			}
