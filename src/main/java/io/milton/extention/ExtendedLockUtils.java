...
public final class ExtendedLockUtils {
    ...
    private static final Logger log = LoggerFactory.getLogger(ExtendedLockUtils.class);
    ...
    public static void respondLocked(LockToken tok, Request request, Response response) {
        ...
        writer.newLine();
        ExtendedLockUtils.appendType(writer, tok.info.type);
        ExtendedLockUtils.appendScope(writer, tok.info.scope);
        ExtendedLockUtils.appendDepth(writer, tok.info.depth);
        ExtendedLockUtils.appendOwner(writer, tok.info.lockedByUser);
        ExtendedLockUtils.appendTimeout(writer, tok.timeout.getSeconds());
        ExtendedLockUtils.appendTokenId(writer, tok.tokenId);
        String url = DefaultPropFindPropertyBuilder.fixUrlForWindows(request.getAbsoluteUrl());
        ExtendedLockUtils.appendRoot(writer, url);
